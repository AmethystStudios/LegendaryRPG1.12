...
public class ScrollRecall extends Item implements IHasModel{
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		if (world.isRemote) {
			...
			//world.playSound(player, player.getPosition(), LRPGSoundHandler.SCROLL_RECALL_TELEPORT, SoundCategory.NEUTRAL, 1.0F, 1.0F);
		...
		for (int countparticles = 0; countparticles <= 20; ++countparticles) {
			world.spawnParticle(EnumParticleTypes.FLAME, player.posX + (rand.nextDouble() - 0.5D) * (double) player.width,
					player.posY + rand.nextDouble() * (double) player.height
							- (double) player.getYOffset(),
