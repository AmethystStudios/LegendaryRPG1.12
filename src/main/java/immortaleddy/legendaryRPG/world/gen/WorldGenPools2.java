	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		// Otherwise
			...
			for (int j = 0; j < i; ++j) {
				...
				for (int l = 1; l < 15; ++l) {
					// Identical to the above loop. This means that for each time the above loop
					// above
					// runs, the loop below run 14 times as well. If each loop runs 13 times, then
					// the
					// loop below will run 196 times
					for (int i1 = 1; i1 < 15; ++i1) {
						// This loop should run 6 times for each time the above loop runs, for a total
						// of 1176 times
						for (int j1 = 1; j1 < 7; ++j1) {
							// This for loop will run for a total of 1176 times, re-running the math each
							// time with the new numbers.
